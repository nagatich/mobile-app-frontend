...
export const getModels = async (name: string) => {
  const data = await request('GET', `get_model_list/${name}/`)
  return data
}

export const getGenerations = async (name: string, model: string) => {
  const data = await request('GET', `get_generation_list/${name}/${model}/`)
  return data.map((item: any) => transformGeneration(item))
}

export const getModifications = async (name: string, model: string, generation: string) => {
